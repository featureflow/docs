...
```groovy
implementation 'io.featureflow:featureflow-java-sdk:1.3.1'
```
...
_Requires `featureflow-java-sdk >= 1.3.1` — see [SDK Compatibility](/sdk-compatibility)._

If a variant has a [JSON config value](/manage-feature-variants#json-configuration-values) set in the dashboard, use `jsonValue()` to read it as a Gson `JsonElement`:

```java
import com.google.gson.JsonElement;
import com.google.gson.JsonObject;

JsonElement config = featureflow.evaluate("checkout-theme", user).jsonValue();

if (config != null && config.isJsonObject()) {
    JsonObject theme = config.getAsJsonObject();
    String color = theme.get("color").getAsString();
    applyTheme(color);
}
```

`jsonValue()` returns `null` if the resolved variant has no JSON config set. Pass a default `JsonElement` to avoid null-checking at every call site:

```java
JsonElement fallback = JsonParser.parseString("{\"color\":\"#000000\"}");
JsonElement config = featureflow.evaluate("checkout-theme", user).jsonValue(fallback);
```

`jsonValue()` is independent of `value()`/`is()`/`isOn()`/`isOff()`, which continue to work with the plain variant key string.
...
| `evaluate(featureKey, user).jsonValue()` | Returns the variant's JSON config value as a `JsonElement`, or `null` if it has none |
| `evaluate(featureKey, user).jsonValue(defaultValue)` | As above, but returns `defaultValue` instead of `null` |
