...
Firstly, Get your Featureflow account at [featureflow.com](https://www.featureflow.com)
...
_Requires `react-featureflow-client >= 2.2.0` (which depends on `featureflow-client >= 2.2.0`) — see [SDK Compatibility](/sdk-compatibility)._

If a variant has a [JSON config value](/manage-feature-variants#json-configuration-values) set in the dashboard, use the `useJsonValue` hook to read it directly:
...
`useJsonValue` returns `undefined` if the resolved variant has no JSON config set, so always provide a fallback. It re-evaluates automatically whenever the underlying feature updates, the same as `useFeature` and `useFeatures`.

If you need the full evaluation result (variant key, `isOn()`/`isOff()`, and the JSON value together), use `useFeature` instead:
