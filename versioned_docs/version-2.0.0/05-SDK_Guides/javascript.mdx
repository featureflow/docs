...
_Requires `featureflow-client >= 2.2.0` — see [SDK Compatibility](/sdk-compatibility)._

If a variant has a [JSON config value](/manage-feature-variants#json-configuration-values) set in the dashboard, use `jsonValue()` to read it:

```js
var evaluation = featureflow.evaluate('checkout-theme');
var config = evaluation.jsonValue();
...
`jsonValue()` returns `undefined` if the resolved variant has no JSON config set. It's independent of `value()`/`is()`/`isOn()`/`isOff()`, which continue to work with the plain variant key string — calling `jsonValue()` never changes what those return.

TypeScript users can pass a generic to type the result:
...
const config = featureflow.evaluate('checkout-theme').jsonValue<ThemeConfig>();
```
...
```
when the user logs out, clear the user, either call:
`featureflow.updateUser();`
or you may wish to reset to an anonymous user with other values.
...
##### `featureflow.evaluate(featureKey).jsonValue()`
Returns the evaluated variant's JSON config value, if one is set in the dashboard.

| Params | Type | Default | Description |
|---------------|----------|--------------|----------------------------------------------------------------|
| `featureKey*`  | `string` | **`Required`** | The feature key you are targeting |
| **`return`** | `object \| undefined` | | The variant's JSON config value, or `undefined` if it has none |
