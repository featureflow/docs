...
# Targeting features
Featureflow has a unique and powerful approach to targeting your features. You can target your feature variants to any attribute you can pass in from your app.
...
#### The Failover Rule
The failover rule is the catch-all if the feature is enabled any other rules do not match. There will always be a default failover rule, the last 'else' rule in the list.
If there are no other rules then all users go to the variant or variant split defined in this last rule:

![Default Rule](/img/2-0/feature-targeting-failover-rule.png)
...
<img src="/img/2-0/feature-targeting-create-attribute.png" alt="Default Rule" width="50%" />
...
Rules may be reordered, rules evaluated first take prescedence over following rules.

![Default Rule](/img/2-0/feature-targeting-multiple-rules.png)
