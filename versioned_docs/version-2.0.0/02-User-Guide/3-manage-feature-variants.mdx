...
Once saved, the JSON value is available to your application through the SDKs — see [Node.js](/nodejs-sdk#json-configuration-values), [JavaScript](/javascript-client#json-configuration-values), [React](/reactjs-client#json-configuration-values), and [Java](/java-sdk#json-configuration-values) for examples.

:::note
JSON config values require a minimum SDK version. Check the [SDK Compatibility](/sdk-compatibility) table before relying on this feature — older SDKs still evaluate the variant key correctly, they just won't have a method to read the JSON payload.
:::
