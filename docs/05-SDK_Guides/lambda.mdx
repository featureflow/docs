...
Firstly, we get some context from the incoming request. From the incoming request we grab one Lambda injected header and one custom header, `country` and `cohort`.

We will use thse values in our feature evaluations.
...
We call the `await` function to ensure that featureflow is ready. This covers for the case of the very first lambda invocation, where featureflow may still be pulling feature configuration from featureflow.
